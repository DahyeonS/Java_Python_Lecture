...
<script>
	...
	function deleteConfirmJson() {
		const input = confirm("회원을 탈퇴할까요?");
		alert(input);
		if (input) location.href = "deleteAjax.jsp";
		else return;
	};
</script>
...
<a href="memberListJson.jsp">회원목록(JSON)</a> |

<% if (session.getAttribute("id") == null) { %>
	<a href="join.jsp">회원가입</a> |
	<a href="joinAjax.jsp">회원가입(JSON)</a> |
	<a href="login.jsp">로그인</a> |
	<a href="loginAjax.jsp">로그인(JSON)</a> |
	
<% } else { %>
	<a href="update.jsp">정보수정</a> |
	<a href="updateAjax.jsp">정보수정(JSON)</a> |

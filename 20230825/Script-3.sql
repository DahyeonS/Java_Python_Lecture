...
select rental_rate, count(film_id) from film group by rental_rate order by rental_rate;
...
select product, price, (price - discount) net_price from tb_item_coalesce_test;
select product, price, (price - coalesce(discount, 0)) net_price from tb_item_coalesce_test; -- NULL값 처리
...
sum(case when gender = 2 then 1 else 0 end) * 100 as "male/female ratio"
from tb_member_nullif_test;

update tb_member_nullif_test set gender = 1;
